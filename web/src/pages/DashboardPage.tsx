...
import { clubsApi, Club, DashboardStats } from '../lib/api';
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <main className="pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400">{selectedClub?.address || 'Добро пожаловать в панель управления'}</p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div className="bg-gradient-to-br from-white/5 to-white/0 border border-white/10 rounded-2xl p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-gray-400 text-sm">Учеников</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-white/5 to-white/0 border border-white/10 rounded-2xl p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-gray-400 text-sm">Абонементов</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-white/5 to-white/0 border border-white/10 rounded-2xl p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-3xl font-bold">{stats?.today_sessions || 0}</div>
                <div className="text-gray-400 text-sm">Занятий сегодня</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-white/5 to-white/0 border border-white/10 rounded-2xl p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-3xl font-bold">{(stats?.month_revenue || 0).toLocaleString()}</div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          <Link
            to={`/clubs/${selectedClub?.id}/groups`}
            className="group bg-gradient-to-br from-white/5 to-white/0 border border-white/10 hover:border-orange-500/50 rounded-2xl p-6 transition-all hover:-translate-y-1"
          >
          ...
          <Link
            to={`/clubs/${selectedClub?.id}/students`}
            className="group bg-gradient-to-br from-white/5 to-white/0 border border-white/10 hover:border-blue-500/50 rounded-2xl p-6 transition-all hover:-translate-y-1"
          >
            <div className="flex items-center justify-between mb-4">
              <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-cyan-600 rounded-xl flex items-center justify-center">
          ...
          <Link
            to={`/clubs/${selectedClub?.id}/groups`}
            className="group bg-gradient-to-br from-white/5 to-white/0 border border-white/10 hover:border-green-500/50 rounded-2xl p-6 transition-all hover:-translate-y-1"
          >
            <div className="flex items-center justify-between mb-4">
              <div className="w-12 h-12 bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl flex items-center justify-center">
                <CalendarCheck className="w-6 h-6" />
              </div>
              <ChevronRight className="w-5 h-5 text-gray-500 group-hover:text-green-500 group-hover:translate-x-1 transition-all" />
            </div>
        ...
        <div className="bg-gradient-to-br from-white/5 to-white/0 border border-white/10 rounded-2xl p-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="font-medium">{selectedClub?.address || '—'}</div>
            </div>
            <div>
              <div className="text-gray-400 text-sm mb-1">Телефон</div>
              <div className="font-medium">{selectedClub?.phone || '—'}</div>
            </div>
            <div>
              <div className="text-gray-400 text-sm mb-1">Валюта</div>
              <div className="font-medium">{selectedClub?.currency || 'KZT'}</div>
            </div>
          </div>
        </div>
      </main>
...
EOFcat > web/src/pages/DashboardPage.tsx << 'EOF'
import { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../contexts/AuthContext';
import { clubsApi, Club, DashboardStats } from '../lib/api';
